...
export default function QuickWins({ wins }: { wins: ResumeAnalysis["quick_wins"] }) {
  ...
  return (
    <section>
      <h2 className="mb-3 flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-secondary">
        <Zap size={13} className="text-accent-2" aria-hidden="true" />
        Quick Wins
      </h2>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {wins.slice(0, 3).map((win, i) => (
          <div
            key={i}
            className="relative rounded-2xl border border-border glass p-5 pl-6 transition-colors duration-150 hover:border-border-bright hover:bg-elevated"
          >
